    ...
    $.fn.extend({
        paging: function(options) {
            ...
            var defaults = {
                ...
                fontWeight: 400, //字体宽度
                currentFontWeight: 400, //当前选中页的字体粗细H
                ...
                jumpFontHoverColor: "#fff", // 跳转内部标签的悬浮字体颜色  
                currentPage: 1, //当前页
                centerBgColor: "#fff", //中间页数按钮的底色
                centerHoverBgColor: "#0094D5", //中间鼠标悬浮的的底色
                otherBgColor: "#fff", //两边按钮的底色
                otherHoverBgColor: "#0094D5", //两边鼠标悬浮的背景颜色
                jumpBgColor: "#fff", //跳转a标签按钮的背景色
                jumpHoverBgColor: "#0094D5", //跳转a标签背景的悬浮背景色
                currentBgColor: "#0094D5", //当前选中页的底色
                centerBorder: "1px solid #ccc", //中间按钮的边框
                otherBorder: "1px solid #ccc", //两边按钮的边框
                centerHoverBorder: "1px solid #0094D5", //中间按钮悬浮边框
                otherHoverBorder: "1px solid #0094D5", //两边按钮悬浮边框
                jumpBorder: "1px solid #ccc", //跳转内标签的边框
                jumpHoverBorder: "1px solid #0094D5", //跳转内标签的悬浮边框
                currentBorder: "1px solid #0094D5", //当前选中页的边框
                ...
                idParameter: "pageNo", //传到后台的当前页的id的参数名，这个传值会自动添加在href或ajax的url末尾
            ...
            return this.each(function() {
                ...
                //显示页超出总页数，则等于总页数
                if (opts.showPage > opts.totalPages)
                    opts.showPage = opts.totalPages;
                ...
                if (opts.animation) { //滚动方式
                    ...
                    for (var i = 1; i <= opts.showPage; i++) {
                        if (i == opts.showPage - 1 && opts.showPage < opts.totalPages)
                            $this.find(".centerBtns ul").append('<li><a value=' + i + '>····</a></li>');
                        else if (i == opts.showPage)
                            $this.find(".centerBtns ul").append('<li><a value=' + opts.totalPages + '>' + opts.totalPages + '</a></li>');
                        else
                            $this.find(".centerBtns ul").append('<li><a value=' + i + '>' + i + '</a></li>');
                    }
                }
                ...
                $this.find(".centerBtns a").css({
                    ...
                    $(element).click(function() {
                        ...
                        //首页和末页左右两侧按钮的cursor
                        if ($(element).attr("value") == "1" && opts.totalPages > 1) {
                        ...
                        } else if ($(element).attr("value") == opts.totalPages && opts.totalPages != 1) {
                            $this.find(".otherBtns a").each(function(index, item_other) {
                        ...
                        } else if (opts.totalPages != 0 && opts.totalPages != 1) {
                        ...
                        if (!opts.animation) { //非滚动方式的显示
                            ...
                            //满足条件就刷新分页
                            if (refresh > 0 && opts.showPage < opts.totalPages) {
                                var newClickPage;
                                if (opts.showPage % 2 != 0)
                                    newClickPage = opts.totalPages - (middle - 1);
                                else
                                    newClickPage = opts.totalPages - middle;
                                    ...
                                    if (clickPage < newClickPage) {
                                        switch (i) {
                                            ...
                                            case (opts.showPage - 1):
                                                $this.find(".centerBtns a").eq(i).text(opts.totalPages);
                                                $this.find(".centerBtns a").eq(i).attr({ "value": opts.totalPages });
                                                if (opts.submitStyle == "href" && initial)
                                                    $this.find(".centerBtns a").eq(i).attr({ "href": opts.url + connect + opts.idParameter + "=" + opts.totalPages });
                                                break;
                                ...
                                for (var i = 0; i < opts.showPage; i++) {
                                    if (i == opts.showPage - 1) {
                                        ...
                                        if (opts.submitStyle == "href" && initial)
                                            $this.find(".centerBtns a").eq(i).attr({ "href": opts.url + connect + opts.idParameter + "=" + opts.totalPages });
                                    } else if (i == opts.showPage - 2 && opts.showPage < opts.totalPages) {
                                        $this.find(".centerBtns a").eq(i).text("····");
                                        var pageIndex = i + 1;
                                        $this.find(".centerBtns a").eq(i).attr({ "value": pageIndex });
                                        if (opts.submitStyle == "href" && initial)
                            ...
                            //动画
                            if (slidePage > 0 && last < opts.totalPages) { //判断是否符合滚动规则 往右

                                var beforeLast = last;

                                //设置参数
                                last += slidePage;
                                if (last > opts.totalPages) { //判断末尾移动长度大于的情况
                                    last = opts.totalPages;
                                    slidePage = opts.totalPages - beforeLast;
                                }
                                first += slidePage;
                                middle += slidePage;
                ...
                if (!!opts.gapWidth) {
                    ...
                    centerAllWidth += (opts.totalPages + 1) * (opts.gapWidth * 2) - (opts.gapWidth * 2);
                }
                ...
                $this.find(".otherBtns a").each(function(index, element) {
                    $(element).click(function() {
                        if (opts.animation) { //因为a标签可能比较多，故不用下面的遍历查找方法
                            ...
                            } else if ($(element).attr("value") == "下一页" && opts.currentPage != opts.totalPages) {
                                if (opts.submitStyle == "href")
                            ...
                            } else if ($(element).attr("value") == "尾页" && opts.currentPage != opts.totalPages) {
                                if (opts.submitStyle == "href")
                ...
                $this.find(".jumpBtns>input").on("keyup", function() {
                    if (/^[1-9]\d*$/.test($(this).val()) || $(this).val() == "") {
                        if ($(this).val() != "" && parseInt($(this).val()) <= opts.totalPages) {
                ...
                $this.find(".jumpBtns>a").click(function() {
                    if (!/^[1-9]\d*$/.test($this.find(".jumpBtns>input").val()) || parseInt($this.find(".jumpBtns>input").val()) > opts.totalPages) {
                    ...
                    if (opts.animation)
                        ...
                        if ($this.find(".jumpBtns>input").val() == opts.totalPages) {
                            $this.find(".centerBtns a").last().trigger("click");
