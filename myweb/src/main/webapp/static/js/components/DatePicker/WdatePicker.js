...
(function() {
	...
	A = L();
	if (_.$wdate)
		M(A + "skin/WdatePicker.css?v=20170524@Bolin");
	V = X;
	if (_.$crossFrame) {
		try {
	...
	function U(E, _) {
		...
		if (_) {
			...
			// 选择Day时重置时分秒
			E.dchanging = function(){
				console.log('bolin')
				parent.document.activeElement.contentDocument.querySelector('input.tB').value = '0';
				parent.document.activeElement.contentDocument.querySelectorAll('input.tE')[0].value = '0';
				parent.document.activeElement.contentDocument.querySelectorAll('input.tE')[1].value = '0';
				$dp.cal.newdate.H = 0;
				$dp.cal.newdate.m = 0;
				$dp.cal.newdate.s = 0;
				/*$dp.dt.H = 0;
			...
			// 	var monthStartDayEl = document.getElementById(gregorianEl.   nnnnnnnnnn+"Start");
			// 	var monthEndDayEl = document.getElementById(gregorianEl.name+"End");
	...
	function K(H, $) {
		...
		if (!$dp.dd
			...
			var B = "<iframe src=\""
					+ _.$dpPath
					+ "My97DatePicker.htm?v=20170524@Bolin\" frameborder=\"0\" border=\"0\" scrolling=\"no\"></iframe>";
			if ($dp.eCont) {
